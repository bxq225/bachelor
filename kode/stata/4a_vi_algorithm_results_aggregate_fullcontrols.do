*************************************************************
**** 4a vi. NH Correction: impact on aggregate real consumption 
*************************************************************

* i) with 1984 as base
use "$resrootdata/nh_percentiles_fullcontrols.dta", clear
foreach i in y qu q {
	replace `i'=exp(`i')
}
...
* ii) results 
use "$resrootdata/nh_percentiles_reverse_fullcontrols.dta", clear
...
scatter pc_dev_real_cons_1984 ref_yr || scatter pc_dev_real_cons ref_yr , msymbol(T) xtitle("Year") ytitle("Bias in Aggreate Real Consumption")  graphregion(color(white)) xlabel(1984(5)2019) legend(order(1 "1984 base prices" 2 " 2019 base prices") rows(1)) ylabel(-2.5(0.5)0)
graph export "$resrootfig/Fig6Bii.pdf", as(pdf) replace
...
graph export "$resrootfig/FigE5Bii.pdf", as(pdf) replace
