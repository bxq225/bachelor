...
***************************************
**** 14ai NH Correction: run algorithm 
***************************************
...
keep ref_yr tot_expn laspeyres_t_tp1 inc decile quintile laspeyres_t_tp1_basic paasche_tm1_t reverse_laspeyres_t_tm1
duplicates drop
...
merge 1:1 ref_yr inc_percentile using "$resrootdata/nh_percentiles_RCF.dta", keepusing(q)
rename q q_guess
	...
	* define tolerance until convergence 
	...
	while `z' > 0.00001 {
		...
		* now compute real consumption, accounting for lambda
		replace q  = Lq + (y-Ly-p)/(1+(1/2)*(Lam_t+Lam_tminus1))  if ref_y==`i'
...
* xi) save file 
save "$resrootdata/nh_percentiles_RCF_order2.dta", replace
