...
****************************************************************
**** 13b i. NH Correction: run algorithm with final period as base
****************************************************************
...
foreach t of numlist 1985(1)2018 {
	...
	* update variables we need for the next period: 
	replace Lq  = L.q  if ref_yr==`t'+2
	replace Lqu = L.qu if ref_yr==`t'+2 // also compute uncorrected
...
* xi) save file 
save "$resrootdata/nh_percentiles_reverse_RCF.dta", replace
