...
bysort ref_yr husstand: egen double laspeyres_t_tp1=sum(expn_shr_t*log(inflation_t_tplus1))
...
bysort ref_yr husstand: egen double reverse_laspeyres_t_tm1=sum(expn_shr_t*log(inflation_t_tminus1^(-1)))
...
bysort ref_yr husstand: egen double paasche_tm1_t=sum(expn_shr_t*(inflation_t_tminus1)^(-1))
replace paasche_tm1_t=(paasche_tm1_t)^(-1)
bysort ref_yr husstand: egen double laspeyres_t_tp1_basic=sum(expn_shr_t*inflation_t_tplus1)

* keep only data we need
keep ref_yr tot_expn laspeyres_t_tp1 husstand paasche_tm1_t laspeyres_t_tp1_basic reverse_laspeyres_t_tm1  
duplicates drop

* iv) define variables we need for the algorithm, reversing the order of time
tsset husstand ref_yr 
gen y=log(tot_expn)
gen Ly=L.y
