...
scatter annual_bias_percent_2003prices ref_yr || scatter annual_bias_percent ref_yr , msymbol(T) xtitle("") ytitle("Aarlig bias i reel forbrugsvækst, %")  graphregion(color(white)) xlabel(2003(2)2022) legend(order(1 "2003 base prices" 2 " 2022 base prices") rows(2)) 
...
graph bar cum_growth_qu cum_growth_q_2003prices cum_growth_q, ascategory ytitle("Cumulative Growth, 2003-2019, %") graphregion(color(white)) blabel(bar, position(inside) format(%9.2f) color(white)) yvar(relabel(1 "Uncorrected" 2 "Corrected, 2003 prices" 3 "Corrected, 2019 prices")) yscale(alt range(28 30))
