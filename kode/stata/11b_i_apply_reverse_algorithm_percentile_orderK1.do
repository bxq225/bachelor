...
****************************************************************
**** 11bi NH Correction: run algorithm with final period as base
****************************************************************

** this file studies the result with a polynomial of order 1
...
reg p Lq1 if ref_yr==1985, r
* save coefficients
matrix b_t2 = e(b)
replace beta1 = b_t2[1,1]

* generate lambda: 
replace Lam = beta1 if ref_yr==1985
...
foreach t of numlist 1986(1)2019 {
	...
	* regress price index on real income at household level
	reg p Lq1 if ref_yr==`t', r
	* save coefficients
	matrix b_t2 = e(b)
	replace beta1 = beta1+b_t2[1,1] if ref_yr>=`t'

	* generate lambda: 
	replace Lam = beta1 if ref_yr>=`t'
...
* xi) save file 
save "$resrootdata/nh_percentiles_reverse_orderK1.dta", replace
