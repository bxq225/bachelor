...
drop if missing(inlfation_t_tminus1)
...
bysort ref_yr: egen double paasche_tm1_t=sum(expn_shr_t*(inlfation_t_tminus1)^(-1))
replace paasche_tm1_t=1/paasche_tm1_t
bysort ref_yr: egen double laspeyres_t_tp1=sum(expn_shr_t*inlfation_t_tplus1)
* geom laspeyres index:
bysort ref_yr: egen geom_laspeyres_t_tp1=sum(expn_shr_t*log(inlfation_t_tplus1))
...
gen double cum_`i'_t_tp1 = `i'_t_tp1 if ref_y==2006
replace cum_`i'=cum_`i'_t_tp1[_n-1]*`i'_t_tp1 if ref_y>2006
}
...
merge 1:1 ref_yr using "$dataroot/Pris_Indeks.csv"
keep if _merge==3 
drop _merge
...
* namely: pensions & social security + life & personal insurance 
drop if missing(inlfation_t_tminus1)
drop if missing(inlfation_t_tplus1)
