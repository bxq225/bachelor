...
keep ref_yr tot_expn laspeyres_t_tp1 paasche_tm1_t inc decile quintile ref_yr_o
duplicates drop
...
replace p_l = -p_l // we reverse time 
replace p_l = 0 if ref_yr==1984 // need to set prices at 0 at the beginning by symmetry relative to previous code with forward time
...
foreach i of numlist 1985(1)2019 {
	...
	* iterate until convergence 
	while `z' > 0.0000001 {
		...
		replace q_update = q + 1/(1+Lambda_t)*(y - Ly_of_q - Pi_tminus1_of_Ly_of_q) if ref_yr==1985
	...
	* get real consumption at time t without NH correction
	replace qu  = Lqu + (y-Ly-p_lasp_hat)  if ref_yr==1985
...
foreach i of numlist 1986(1)2019 {
	...
	* iterate until convergence 
	while `z' > 0.0000001 {
		...
		replace q_update = q + 1/(1+Lambda_t)*(y - Ly_of_q - Pi_tminus1_of_Ly_of_q) if ref_yr==`i'
	...
	* get real consumption at time t without NH correction
	replace qu  = Lqu + (y-Ly-p_lasp_hat)  if ref_yr==`i'
...
* xi) save file 
save "$resrootdata/nh_percentiles_refined_order1_reverse.dta", replace
