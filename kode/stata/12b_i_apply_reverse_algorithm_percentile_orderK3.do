...
****************************************************************
**** 12bi NH Correction: run algorithm with final period as base
****************************************************************
...
reg p Lq1 Lq2 Lq3 if ref_yr==1985, r
* save coefficients
matrix b_t2 = e(b)
replace beta1 = b_t2[1,1]
replace beta2 = b_t2[1,2]
replace beta3 = b_t2[1,3]

* generate lambda: 
replace Lam = beta1 + beta2*2*Lq + beta3*3*(Lq)^2 if ref_yr==1985
...
foreach t of numlist 1986(1)2019 {
	...
	* regress price index on real income at household level
	reg p Lq1 Lq2 Lq3 if ref_yr==`t', r
	* save coefficients
	matrix b_t2 = e(b)
	replace beta1 = beta1+b_t2[1,1] if ref_yr>=`t'
	replace beta2 = beta2+b_t2[1,2] if ref_yr>=`t'
	replace beta3 = beta3+b_t2[1,3] if ref_yr>=`t'
	
	* generate lambda: 
	replace Lam = beta1 + beta2*2*Lq + beta3*3*(Lq)^2 if ref_yr>=`t'
...
* xi) save file 
save "$resrootdata/nh_percentiles_reverse_orderK3.dta", replace
