...
scatter pc_dev_real_cons ref_yr || scatter pc_dev_real_cons_first ref_yr , xtitle("Year") ytitle("Bias in Aggreate Real Consumption" "= (Nh-Naive - Nh-True)/Nh-Naive*100 ")  graphregion(color(white)) xlabel(2003(5)2022) 
...
scatter pc_dev_real_cons_2003prices ref_yr || scatter pc_dev_real_cons_first_f ref_yr, msymbol(T) || scatter pc_dev_real_cons_2nd_order_f ref_yr, msymbol(D) xtitle("Year") ytitle("Bias in Average Real Consumption")  graphregion(color(white)) xlabel(2003(5)2019) legend(order(1 "First-order NH correction, Geometric Laspeyres" 2 "First-order NH correction, Fisher" 3 "Second-order NH correction, Fisher") rows(3)) 
graph export "$resrootfig/Fig6Ai.pdf", as(pdf) replace

* plot the annual bias 
foreach i in annual_bias_percent_2003prices annual_bias_percent_f annual_bias_percent_2nd_order Lambda_rescaled_2nd_order {
	replace `i'=`i'*100
	replace `i'=0 if ref_yr==2003
}

scatter annual_bias_percent_2003prices ref_yr || scatter annual_bias_percent_f ref_yr || scatter Lambda_rescaled_2nd_order ref_yr, ///
	xtitle("Year") ///
	ytitle("Annual Bias in Real Consumption Growth, %") ///
	graphregion(color(white)) ///
	xlabel(2003(5)2019) ///
	legend(order(1 "First-order NH correction, Geometric Laspeyres" 2 "First-order NH correction, Fisher" 3 "Second-order NH correction, Fisher") rows(3)) 
...
scatter pc_dev_real_cons ref_yr || scatter pc_dev_real_cons_first ref_yr , xtitle("Year") ytitle("Bias in Aggreate Real Consumption" "= (Nh-Naive - Nh-True)/Nh-Naive*100 ")  graphregion(color(white)) xlabel(2003(5)2019) 
