...
graph export "$resrootfig/FigE2ii_alder_2003.pdf", as(pdf) replace
...
twoway ///
    ...
    (qfit annual_bias_percent a if ref_yr_o==2003), ///
   xtitle("", margin(t=10)) ///
...
graph export "$resrootfig/Fig4Aii_alder.pdf_2003", as(pdf) replace
...
* fig 4Bii
scatter pc_dev_real_cons a if ref_yr_o==2003 || qfit pc_dev_real_cons a if ref_yr_o==2003, xtitle("") ytitle("Bias in 2003 Real Consumption Level, %")  graphregion(color(white)) xlabel(1 "under 30 aar" 2 "30-44 aar" 3 "45-59 aar" 4 "60 - 74 aar" 5 "75 aar og derover") legend(off)
graph export "$resrootfig/Fig4Bii_alder_2003.pdf", as(pdf) replace
...
graph export "$resrootfig/FigE3ii_alder_2003.pdf", as(pdf) replace
