...
twoway connected p_naive a if ref_yr==2022 || connected p_full a if ref_yr==2022, xtitle("Pre-tax Income Percentile") ytitle("Geometric Index in 2022 (1984=100)")  graphregion(color(white)) xlabel(1(1)5) ///
...
* fig 12d
scatter pc_dev_real_cons a if ref_yr==2022 || lfit pc_dev_real_cons a if ref_yr==2022, xtitle("a") ytitle("Bias i 2022 reel forbrugs niveau, %")  graphregion(color(white)) xlabel(1(1)5) legend(off)
...
twoway connected change_real_exp a || connected bias_pp a, xtitle("Pre-tax Income Percentile") ytitle("Bias in Cumulative Real Consumption Growth" "1984-2022, pp (% of 1984 Nominal Expenditure)")  graphregion(color(white)) xlabel(1(1)5) legend(order(1 "From Group-specific Index" 2 "From NH correction") rows(2))
graph export "$resrootfig/FigE3i_alder.pdf", as(pdf) replace
